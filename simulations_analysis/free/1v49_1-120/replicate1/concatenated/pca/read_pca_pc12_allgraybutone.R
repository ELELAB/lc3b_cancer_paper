...
pca_proj$FF[300002:400001]<-"amber99star-ildn"
pca_proj$FF[400002:500001]<-"amber99star-ildn-q"
pca_proj$FF[500002:600001]<-"amber99star-nmr"
pca_proj$FF[600002:700001]<-"a14"
pca_proj$FF[700002:800001]<-"rsff1"
pca_proj$FF[800002:900001]<-"rsff2"
pca_proj$FF[900002:1000001]<-"amber99sb-disp"
...
pca_proj_sub_color10 <- pca_proj_sub[pca_proj_sub$FF==FFClasses[10],]
pca_proj_sub_color10["FF_color"] <- FFClasses[10]
pca_plot10 <- rbind(pca_proj_sub,pca_proj_sub_color10) 


# xlim=c(-2.5,2.5), ylim=c(-2.5,2.5), #c(-2.5,4.1), ylim=c(-2.5,4.1)
FFallplot <- ggscatter(pca_proj_sub, x = "PC1", y = "PC2", #shape = 1,
                       color = "FF", palette = c("charmm22star"="gray60", "charmm27"="#E69F00","charmm36"="#56B4E9","amber99star-ildn"="#F0E442",
...
FFallplot <- FFallplot+ annotate("text", x = -10, y = 20, label = paste0("All FFs")) # x = -1, y = 2,
...
FF1plot <- FF1plot+ annotate("text", x = -10, y = 20, label = paste0("CHARMM22*"))
...
FF2plot <- ggscatter(pca_plot2, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF2plot <- FF2plot+ annotate("text", x = -10, y = 20, label = paste0("CHARMM27"))
...
FF3plot <- ggscatter(pca_plot3, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF3plot <- FF3plot+ annotate("text", x = -10, y = 20, label = paste0("CHARMM36"))
...
FF4plot <- ggscatter(pca_plot4, x = "PC1", y = "PC2",
                     color = "FF_color", palette = c("darkgray","#F0E442"),
                     xlab = "PC1 (Å)", ylab = "PC2 (Å)", show.legend = FALSE,
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF4plot <- FF4plot+ annotate("text", x = -10, y = 20, label = paste0("ff99SB*-ILDN"))
...
FF5plot <- ggscatter(pca_plot5, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF5plot <- FF5plot+ annotate("text", x = -10, y = 20, label = paste0("ff99SB*-ILDN-Q"))
...
FF6plot <- ggscatter(pca_plot6, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF6plot <- FF6plot+ annotate("text", x = -10, y = 20, label = paste0("ff99SBnmr1"))
...
FF7plot <- ggscatter(pca_plot7, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF7plot <- FF7plot+ annotate("text", x = -10, y = 20, label = paste0("ff14SB"))
...
FF8plot <- ggscatter(pca_plot8, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF8plot <- FF8plot+ annotate("text", x = -10, y = 20, label = paste0("RSFF1"))
...
FF9plot <- ggscatter(pca_plot9, x = "PC1", y = "PC2",
                     ...
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF9plot <- FF9plot+ annotate("text", x = -10, y = 20, label = paste0("RSFF2"))

FF9plot <- FF9plot+ annotate("text", x=pca_proj$PC1[800002], y=pca_proj$PC2[800002], label = "*", color="white")


FF10plot <- ggscatter(pca_plot10, x = "PC1", y = "PC2",
                     color = "FF_color", palette = c("darkgray","purple"),
                     xlab = "PC1 (Å)", ylab = "PC2 (Å)",
                     xlim=c(-25,25), ylim=c(-25,25),
                     size = 2, alpha = 0.7)+ border() + theme(legend.position = "none")
FF10plot <- FF10plot+ annotate("text", x = -10, y = 20, label = paste0("a99SB-disp"))

FF10plot <- FF10plot+ annotate("text", x=pca_proj$PC1[900002], y=pca_proj$PC2[900002], label = "*", color="white")
...
ggarrange(FFallplot,FF1plot, FF2plot, FF3plot,
          FF7plot, FF4plot, FF5plot, FF6plot,
          FF9plot, FF10plot, FF8plot, 
          ncol = 4, nrow = 3,
          common.legend = FALSE)
